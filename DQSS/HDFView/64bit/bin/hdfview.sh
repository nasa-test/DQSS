...
HDFVIEW_HOME=../HDFView/64bit #..
export HDFVIEW_HOME

# where Java is installed (requires jdk1.4.x or above), e.g. /usr/jdk1.4.2/bin
JAVAPATH=$JAVA_HOME/bin #/usr/java/jdk-106/jdk/bin
export JAVAPATH
...
if [ ! -d $JAVAPATH ]; then
    ...
    JAVAPATH=../HDFView/64bit/jre/bin #`perl -e "print substr(\"${JAVALOC}\", 0, $LASTSLASH)"`
fi
...
$JAVAPATH/java  -Djava.library.path=$LD_LIBRARY_PATH ncsa.hdf.view.HDFView -root $HDFVIEW_HOME $*
